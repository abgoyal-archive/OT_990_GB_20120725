...
wpa_supplicant_ctrl_iface_init(struct wpa_supplicant *wpa_s)
{
	...
	if (mkdir(dir, S_IRWXU | S_IRWXG) < 0) {
		if (errno == EEXIST) {
			...
			wpa_printf(MSG_ERROR,"mkdir[ctrl_interface]");
			goto fail;
		}
	}

    /* we have to be logical. the wpa_supplicant is now running under AID_WIFI group
       so grp can be nothing else than AID_WIFI.
       parameter 'GROUP' in the config file will be ignored 
    */
  if (gid_str) {
    wpa_printf(MSG_WARNING,".conf command 'GROUP=%s' ignored as group is WIFI",gid_str);
  }
#if 0
	if (gid_str) {
	...
	if (gid_set && chown(dir, -1, gid) < 0) {
		...
		wpa_printf(MSG_ERROR,"chown[ctrl_interface] %s ", strerror(errno));
		goto fail;
	}
	...
	if (priv->sock < 0) {
		...
		wpa_printf(MSG_DEBUG,"socket(PF_UNIX)");
		goto fail;
	}
    ...
    wpa_printf(MSG_DEBUG,"chown[ctrl_interface/%s] %s ", fname, strerror(errno));
		goto fail;
	}

	if (chmod(fname, S_IRWXU | S_IRWXG) < 0) {
		perror("chmod[ctrl_interface/ifname]");
    wpa_printf(MSG_DEBUG,"chmod[ctrl_interface/%s] %s ", fname, strerror(errno));
		goto fail;
	}
	os_free(fname);
