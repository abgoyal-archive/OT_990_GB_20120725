...
#ifdef BT_ALT_STACK
void update_lastseen(bdaddr_t *sba, bdaddr_t *dba)
#else
...
#ifdef BT_ALT_STACK
void link_key_notify(int dev, bdaddr_t *sba, void *ptr)
#else
...
#ifdef BT_ALT_STACK
void pin_code_request(int dev, bdaddr_t *sba, bdaddr_t *dba)
#else
...
#ifdef BT_ALT_STACK
void inquiry_complete(bdaddr_t *local, uint8_t status, gboolean periodic)
#else
