...
/*--------------------------------------------------------------------------------------*/
/*   Author :  Hualan.Su                                                     */
/*   Role   :  Bluetooth bluez                                               */
/*   Reference documents :                                                   */
/*--------------------------------------------------------------------------------------*/
/*    Comments :                                                             */
/*    File     : /external/bluetooth/bluez/src/device.c                      */
/*    Labels   :                                                             */
/*======================================================================================*/
...
/*---------|------------------|--------------------------|------------------------------*/
/*11/28/2011 | Hualan.Su  | PR191778-Hualan.Su-001  |device_remove_bonding when pair error  */
/*---------|------------------|--------------------------|------------------------------*/
/*---------|------------------|--------------------------|------------------------------*/
/*======================================================================================*/
#ifdef HAVE_CONFIG_H
...
#define BT_CLASS_POINTING   0x0580
...
gboolean device_is_weak_linkkey(struct btd_device *device)
{
    ...
    if (str)
    {
        debug ("device_is_weak_linkkey: %s", str);
...
static DBusMessage *set_service_trust (DBusConnection *conn,
...
void discover_services_reply(int err,sdp_list_t * recs) {
	...
	fflush(stdout);
	//pr152814-hualan.su@jrdcom.com-20110822-s
	if(!g_browse_req){
		LOGE("g_browse_req == NULL");
		return;
	}
	//pr152814-hualan.su@jrdcom.com-20110822-e
...
void device_remove_bonding(struct btd_device *device)
{
    ...
    /* Always delete the HID info entry if present. */
    create_name(filename, PATH_MAX, STORAGEDIR, srcaddr, "hidinfo");
...
static void device_set_pairing_failed_reason(struct btd_device *device, uint32_t value)
{
	DBusConnection *conn = get_dbus_connection();

	emit_property_changed(conn, device->path, DEVICE_INTERFACE, "PairingFailed",
...
#ifdef BT_ALT_STACK
	DBG("Just add the profiles for %s", device->path);
...
send_reply:
	...
	if (dbus_message_is_method_call(req->msg, DEVICE_INTERFACE,
					"DiscoverServices"))
#ifdef BT_ALT_STACK
                discover_services_reply(err, req->records);
#else
...
#if 0
    //
    //save the message in our global
    //
    if (conn && msg)
    {
        //
        // save the connection and message to send back async response
        //
        boolean success = 1;
        DBusMessage* reply = dbus_message_new_method_return(msg);
...
void device_bonding_complete(struct btd_device *device, uint8_t status)
{
	...
	if (status) {
        ...
        //PR191778-Hualan.Su-001-Add begin
        #ifdef BT_ALT_STACK
              /* brcm fix CSP474493 */
              device_remove_bonding(device);
        #endif
        //PR191778-Hualan.Su-001-Add end
		return;
	}
...
void device_cancel_bonding(struct btd_device *device, uint8_t status)
{
	...
	if (!bonding)
	{
	    device_set_pairing_failed_reason(device, status);
		return;
    }
