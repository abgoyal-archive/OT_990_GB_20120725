...
int hcid_dbus_user_confirm(bdaddr_t *sba, bdaddr_t *dba, uint32_t passkey)
{
	...
	if (!get_adapter_and_device(sba, dba, &adapter, &device, TRUE))
		return -ENODEV;
//PR178270-yjzhu add for CTS test fail begin
//+++ BRCM_LOCAL : CtsVerifier insecure test failure. Google is insisting no user prompt if "Just Works" model is being used for SSP.
	if (passkey == 0x80000000) //This is used as Bluetooth.Error in the JAVA space
	{
...
//--- BRCM_LOCAL
//PR178270-yjzhu add for CTS test fail end
