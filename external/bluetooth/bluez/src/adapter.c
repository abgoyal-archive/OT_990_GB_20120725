			...
			DBusMessage *msg)
{
    ...
    /* Create pending session before dispatching connectable or discoverable request,
     * because DTUN call calls adapter_mode_changed synchronously.
     */
#ifdef BT_ALT_STACK
	if (msg != NULL) {
...
#ifdef BT_ALT_STACK
static void session_free2(struct session_req *req)
{
...
/* Blocklist support for Gingerbread - start */
#ifdef BT_ALT_STACK
static DBusMessage *set_device_blocked_state(DBusConnection *conn, DBusMessage *msg, const char *dev_addr, gboolean blocked, void *data)
{
	struct btd_adapter *adapter = data;
	int err;
	bdaddr_t bdaddr;
	gboolean device_is_blocked;
	const char *path;

	info("######");
	info("###### set_device_blocked_state +++");
	info("######");

	// Prepare device blocked address
    	str2ba(dev_addr, &bdaddr);
	// get the device blocked state 
	device_is_blocked = read_blocked(&adapter->bdaddr, &bdaddr);
	// get the calling adapter DBUS path
	path = dbus_message_get_path(msg);

	// if blocked request and the device is already blocked then do nothing
	if (TRUE == device_is_blocked && TRUE == blocked)
	{
		info("###### blocked request but the device is already blocked, just return.");
		goto done;
	}
	// if unblocked request and the device has not been blocked then do nothing
	if (FALSE == device_is_blocked && FALSE == blocked)
	{
		info("###### unblocked request but the device has not been blocked, just return.");
		goto done;
	}
	// notify btld
    	err = 0;
	...
	method.block_dev.hdr.len =  sizeof(tDTUN_METHOD_DM_BLOCK_DEV) - sizeof(tDTUN_HDR);
    	method.block_dev.block = (TRUE == blocked) ? 1 : 0;
	memcpy(&method.block_dev.bdaddr, bdaddr.b, 6);
	dtun_client_call_method(&method);

	// save the device blocked state
	err = write_blocked(&adapter->bdaddr, &bdaddr, blocked);
	if (err < 0)
		error("write_blocked(): %s (%d)", strerror(-err), -err);

	emit_property_changed(connection, path, ADAPTER_INTERFACE, "Blocked", DBUS_TYPE_BOOLEAN, &blocked);

done:
	return msg ? dbus_message_new_method_return(msg) : NULL;
}

static DBusMessage *get_blocked_state(DBusConnection *conn, DBusMessage *msg, void *data)
{
	DBusMessageIter iter;
	struct btd_adapter *adapter = data;
	  DBusMessage *reply;
	...
	dbus_message_iter_get_basic(&iter, &dev_addr);
	// Prepare device blocked address
    	str2ba(dev_addr, &bdaddr);

	blocked = read_blocked(&adapter->bdaddr, &bdaddr);
	  reply = dbus_message_new_method_return(msg);
	  if (!reply)
		    return NULL;

	  dbus_message_append_args(reply, DBUS_TYPE_BOOLEAN, &blocked,
						    DBUS_TYPE_INVALID);
					...
					DBusMessage *msg, void *data)
{
    ...
    /* Blocklist support for Gingerbread */
#ifdef BT_ALT_STACK
	else 
	{
		int err;

		if (g_str_equal("Blocked", property)) {
			gboolean blocked;
			const char *dev_addr;

			info("######");
			info("###### Adapter setDeviceBlockedState");
			info("######");
			if (dbus_message_iter_get_arg_type(&sub) != DBUS_TYPE_BOOLEAN)
				return invalid_args(msg);

			dbus_message_iter_get_basic(&sub, &blocked);
			...
			dbus_message_iter_get_basic(&sub, &dev_addr);
			info("###### blocked_addr = %s", dev_addr);
			return set_device_blocked_state(conn, msg, dev_addr, blocked, data);
		}
	}
#endif
...
static DBusMessage *resolve_name(DBusConnection *conn, DBusMessage *msg, void *user_data)
{
    char *address;
    bdaddr_t bdaddr;
    struct btd_adapter *adapter = user_data;
    ...
    adapter_ops->resolve_name(0, bdaddr.b);

    return dbus_message_new_method_return(msg);
}

// CK - Btld specific oob support
static DBusMessage *generate_oob_keys(DBusConnection *conn, DBusMessage *msg, void *user_data)
{
    dtun_client_call_id_only(DTUN_METHOD_GENERATE_LOCAL_OOB_KEYS);
...
void dtun_sig_dm_local_oob_keys(tDTUN_DEVICE_SIGNAL *msg)
{
    debug("####");
    debug(__FUNCTION__);
    debug("#### received signal \"LocalOobKeysAvailable\"");
    debug("####");

/* CK -
    msg->local_oob_keys.hash,
    msg->local_oob_keys.randomizer,
*/
    // CK - TODO add handling oob keys when NFC is ready

}
// CK - End - Btld specific oob support
...
static DBusMessage *list_hid_info(DBusConnection *conn, DBusMessage *msg, void *user_data)
{
    ...
    char local_addr[18];
    char hid_list[512];
    const gchar *c_hid_list = hid_list;
    struct btd_adapter *adapter = user_data;
	DBusMessage *reply;
    ...
    hid_list[0] = '\0';
    textfile_foreach(filename, create_hid_list_from_hidinfo, hid_list);
...
static GDBusMethodTable adapter_methods[] = {
	...
	{ "GetBlockedState",  "s", "b",  get_blocked_state},
	{ }
};
...
static GDBusSignalTable adapter_signals[] = {
	...
	{ "DeviceFound",		"sa{sv}"	},
	{ "DeviceDisappeared",		"s"		},
	{ "AgConnectionStatus",	"su"	    },
    { "AgSlcUp",            "s"       },
	{ "LocalOobKeysAvailable",	"ss"		},
	{ "LocalOobKeysAvailable",	"ss"		},
	{ }
};
...
int adapter_stop(struct btd_adapter *adapter)
{
	...
	if (adapter->disc_sessions) {
#ifdef BT_ALT_STACK
		g_slist_foreach(adapter->disc_sessions, (GFunc) session_free2,
				NULL);
#else
	...
	while (adapter->connections) {
		...
		adapter->connections = g_slist_remove(adapter->connections, device);
#else
		adapter_remove_connection(adapter, device, 0, 0);
#endif
	}
...
static void emit_second_conn_info(const char *path, const char *address,
				uint32_t result)
{
    DBusMessage *signal;
    DBusMessageIter iter;
    ...
    dbus_message_append_args(signal, DBUS_TYPE_STRING, &address, DBUS_TYPE_UINT32, &result, DBUS_TYPE_INVALID);
    g_dbus_send_message(connection, signal);
}


static void emit_slc_up(const char *path, const char *address)
{
    DBusMessage *signal;
    DBusMessageIter iter;
    va_list var_args;
    ...
    dbus_message_append_args(signal, DBUS_TYPE_STRING, &address, DBUS_TYPE_INVALID);
    g_dbus_send_message(connection, signal);
}


void adapter_emit_slc_up(struct btd_adapter *adapter, const bdaddr_t *peer) 
{
    char peer_addr[18];
    const char *paddr = peer_addr;
    ba2str(peer, peer_addr);
    info("*********************** sending slc up for address %s", paddr);
    emit_slc_up(adapter->path, paddr);
}

void adapter_emit_second_conn_info(struct btd_adapter *adapter, const bdaddr_t *peer, uint8_t result) 
{
    char peer_addr[18];
    const char *paddr = peer_addr;
    ba2str(peer, peer_addr);
    uint32_t res= result;
    info("*********************** sending info for second ag- address %s result %d strlen is %d", paddr, result, strlen(paddr));
    emit_second_conn_info(adapter->path, paddr, result);
}
...
void adapter_remove_connection(struct btd_adapter *adapter,
				struct btd_device *device, uint16_t handle, uint8_t reason)
{
	bdaddr_t bdaddr;
	...
	device_remove_connection(device, connection, handle, reason);
...
gboolean get_service_from_uuid(const char *uuid, char *service)
{
	if (strncasecmp(PBAP_UUID, uuid, sizeof(PBAP_UUID)) == 0) {
		strcpy(service, "service_pbap");
	} else if (strncasecmp(OPP_UUID, uuid, sizeof(OPP_UUID)) == 0) {
		strcpy(service, "service_opp");
	} else if (strncasecmp(FTP_UUID, uuid, sizeof(FTP_UUID)) == 0) {
		strcpy(service, "service_ftp");
	} else if (strncasecmp(HID_UUID, uuid, sizeof(HID_UUID)) == 0) {
		strcpy(service, "service_hid");
	} else if (strncasecmp(SERIAL_PORT_UUID, uuid, sizeof(SERIAL_PORT_UUID)) == 0) {
		strcpy(service, "service_spp");
	} else if (strncasecmp(DIALUP_NET_UUID, uuid, sizeof(DIALUP_NET_UUID)) == 0) {
		strcpy(service, "service_dun");
	} else if (strncasecmp(SIM_ACC_UUID, uuid, sizeof(SIM_ACC_UUID)) == 0) {
		strcpy(service, "service_sap");
	} else if (strncasecmp(MAP_UUID, uuid, sizeof(MAP_UUID)) == 0) {
		strcpy(service, "service_map");
	} else if (strncasecmp(MSE_UUID, uuid, sizeof(MSE_UUID)) == 0) {
		strcpy(service, "service_mse");
	} else if (strncasecmp(MNS_UUID, uuid, sizeof(MNS_UUID)) == 0) {
		strcpy(service, "service_mns");
	} else {
		*service = 0;
		return FALSE;
	}
					...
					service_auth_cb cb, void *user_data)
{
    ...
    /* Instead of writing/reading the GLOBAL_TRUST, we now write and read per service */
#if 0
...
#else
    {
        ...
        if (get_service_from_uuid(uuid, service)) {
            trusted = read_trust(&adapter->bdaddr, address, service);
        }
        info("btd_adapter_authorize: read_trust: service:%s trust_value:%d",
             service, trusted);
        if (trusted) {
            adapter->auth_idle_id = g_idle_add_full(G_PRIORITY_DEFAULT_IDLE,
                    auth_idle_cb, auth,
