...
/* NOTE : these needs to match the definitions in btld_prop.h and dtun.h */
...
static int btl_send_on_off_signal( int sig )
{
    ...
    LOGI("btl_send_on_off_signal():Sending signal %d to bt daemon %d ", sig , pid );
...
static void bt_start_daemons( const BTLD_CONTROL_ACTION_t ctl_act )
{
    ...
    LOGI( "bt_start_daemons( ctl_act: %d ), soft_on_off: %d:Starting daemons...", ctl_act, soft_on_off );

    if ( (1==soft_on_off) && (ctl_act==BTLD_CTRL_NO_SW) )
    {
        LOGI( "soft_on_off enabled, ignore ctrl" );
        return;
    }
	if ( (0==soft_on_off) && (ctl_act==BTLD_SIG_NO_SW) )
    {
        LOGI( "soft_on_off is not enabled, ignore signal" );
        return;
    }
    ...
    if (is_hciattach_enabled())
    {
        ...
        if (!is_emulator_context("hciattach"))
        {
            ...
            LOGI("Now stopping hciattach daemon");
    ...
    if ( (ctl_act==BTLD_SIG) || (ctl_act==BTLD_SIG_NO_SW) )
    {
        ...
        {
            LOGE( "Fail: Invalid BTLD PID. Unable to send BT ENABLE signal " );
            return;
        }
    ...
    LOGI("bluetoothd start retry count %d/%d",
         START_DAEMON_PROPERTY_CHECK_TIMEOUT_RETRY_COUNT-retry_cnt,
         START_DAEMON_PROPERTY_CHECK_TIMEOUT_RETRY_COUNT);

    if (retry_cnt == 0) {
        LOGE("bluetoothd start timed out");
        return;
    }

    LOGI("bt_start_daemons(): Starting daemons... Done!");
}
...
static void bt_stop_daemons( const BTLD_CONTROL_ACTION_t ctl_act )
{
    ...
    LOGI(" bt_stop_daemons( ctl_act: %d ), soft_on_off: %d: Stopping daemons...", ctl_act, soft_on_off );

    if ( (1==soft_on_off) && (ctl_act==BTLD_CTRL_NO_SW) )
    {
        LOGI( "bt_stop_daemons(): soft on_off enabled ignore ctrl off" );
        return;
    }

	if ( (0==soft_on_off) && (ctl_act==BTLD_SIG_NO_SW) )
    {
        LOGI( "bt_stop_daemons(): soft on_off is not enabled ignore signal off" );
        return;
    }
    LOGI("Stopping bluetoothd...");
    if (property_set("ctl.stop", "bluetoothd") < 0) {
        LOGE("Failed to stop bluetoothd");
        return;
    }
    ...
    if (ctl_act==BTLD_SIG || ctl_act==BTLD_SIG_NO_SW) {
        if ( 0 > btl_send_on_off_signal( BTL_BT_FM_ON_OFF_SIGNAL ) )
        {
            LOGE( "Fail: Invalid BTLD PID. Unable to send BT DISABLE signal " );
            return;
        }
        ...
        if (property_set("ctl.stop", "btld") < 0) {
            LOGE("Failed to stop btld");
            return;
        }
    }

    // Wait until both bluetoothd and btld daemons are down
    LOGI("Wait until all daemons are stopped, or timed out...");
    ...
    while ((bluetoothd_stopped == 0 || btld_stopped == 0) && --retry_cnt) {
		...
		if (bluetoothd_stopped == 0 && is_bluetoothd_stopped() == 1) {
						LOGI("bluetoothd has stopped");
				if (dtun_property_is_active(DTUN_PROPERTY_HCID_ACTIVE) == 1) {
								LOGI("But dtun_property_hcid_active is still 1. So clearing it...");
								property_set(DTUN_PROPERTY_HCID_ACTIVE, "0");
				}
						bluetoothd_stopped = 1;
		}
    ...
    LOGW("Daemon stop waiting count %d/%d",
    ...
    if (retry_cnt == 0 && (bluetoothd_stopped == 0 || btld_stopped == 0)) {
        if (bluetoothd_stopped == 0) {
            LOGW("bluetoothd stop timed out");
            property_set(DTUN_PROPERTY_HCID_ACTIVE, "0");
        }
        if (btld_stopped == 0) {
            LOGW("btld stop timed out");
...
int BTOFF_FMOFF_handling(eBT_FM_STATE_ACTION st)
{
    ...
    LOGI("BTOFF_FMOFF_handling");
	BT_LOCK_MUTEX()
	switch(st)
	{
	case BT_ON:
        // load btld for BT
	    LOGI("BTOFF_FMOFF_handling : receiving BT_ON");
        ...
        * depending on whether on/off is enabled or not. */
	    ...
	    LOGI("New state is BTON_FMOFF_handling");
		break;
	case FM_ON:
        // load btld for FM
        LOGI("BTOFF_FMOFF_handling : receiving FM_ON");
	    ...
	    LOGI("New state is BTOFF_FMON_handling");
		break;
	default: break;
	}
	BT_UNLOCK_MUTEX()
...
// Only BT_OFF and FM_ON are handled in this state
	...
	case BT_OFF:
	    ...
	    LOGI("BTON_FMOFF_handling : receiving BT_OFF");
        property_set(BRCM_PROPERTY_BT_ACTIVATION, "0");
...
// Only BT_ON and FM_OFF are handled in this state
    LOGI("BTOFF_FMON_handling");
	BT_LOCK_MUTEX()
	switch(st)
	{
	case BT_ON:
        // As FM is on and btld is already loaded, so just initialize BT
	    LOGI("BTOFF_FMON_handling : receiving BT_ON");
        property_set(BRCM_PROPERTY_BT_ACTIVATION, "1");
	...
	case FM_OFF:
	    ...
	    LOGI("BTOFF_FMON_handling : receiving FM_OFF");
		/*
...
int BTON_FMON_handling(eBT_FM_STATE_ACTION st)
{
    ...
    LOGI("BTON_FMON_handling");
	BT_LOCK_MUTEX()
	switch(st)
	{
	case BT_OFF:
        // as FM is still on, so keep btld loaded
	    LOGI("BTON_FMON_handling : receiving BT_OFF");
		bt_current_state = BTOFF_FMON;
        property_set(BRCM_PROPERTY_BT_ACTIVATION, "0");
	...
	case FM_OFF:
	    ...
	    LOGI("BTON_FMON_handling : receiving FM_OFF");
...
int dtun_property_is_active(char *property) {return 1;}
...
int dtun_property_is_active(char *property) {
    ...
    LOGV("dtun_property_is_active : %s=%s\n", property, value);
...
static int set_bluetooth_power(int on) {
    ...
    if (is_emulator_context(""))
    {
        ...
        LOGV("set_bluetooth_power [emul] %d\n", on);      
        bt_emul_enable = on;
        return 0; /* return 0 to allow starting daemons */
    }
...
///////////////////////////////////////////////////////////////////
//
// bt_enable() for frameworks entry
int bt_enable() {
    LOGI("bt_enable...");
#ifdef BT_ONOFF_PROFILE_ENABLED
    {
        ...
        LOGI("PROFILE:bt_enable: Turnaround time: %f seconds", timediff(&start_time, &end_time));
        return ret;
     }
#else
...
///////////////////////////////////////////////////////////////////
//
// bt_disable() for frameworks entry
int bt_disable() {
    int ret;
...
#ifdef BT_ONOFF_PROFILE_ENABLED
    {
        ...
        gettimeofday(&end_time, NULL);
        LOGI("PROFILE:bt_disable: Turnaround time: %f seconds", timediff(&start_time, &end_time));
        return ret;
    }
#else
    ret = BT_FM_state_handling[bt_current_state](BT_OFF);
